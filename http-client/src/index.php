...
- 1)];
 ...
 curl_close($myCurl);
 $response_array = json_decode($response, true); //декодирование
json-строки в ассоциативный массив[29]
 var_dump($response_array);//вывод на страницу json-строки
результата
 $base64_code = $response_array['image_base64'];
 $encoding = $response_array['encoding'];
 $image_path = "./images/".generateRandomString().".jpg";//путь к
картинке относительный
 $image_path_full= "/var/www/html/".$image_path; // абсолютный путь
к картинке
 $fp = fopen($image_path_full, "w+");// открытие файла на запись[30]
 fwrite($fp, base64_decode($base64_code)); //запись в файл
декодированных байтов[31]
